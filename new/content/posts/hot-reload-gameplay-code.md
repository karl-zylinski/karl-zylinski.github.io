...
I think hot reload is more useful to people who are _making a game_ rather than a general purpose game engine. When making just a game you only program the parts that your game needs, not what someone else _might need_. Therefore there are way less moving parts and layers of code in a pure game. This makes you less likely to run into problems with hot reload.
...
// Returns the pointer to the game memory. When a hot reload occurs, then main
// program needs to get hold of the game memory pointer. After that it can load
// a new game DLL and tell that game DLL to use the same memory by calling
// game_hot_reloaded in the new game DLL and supplying it the GameMemory pointer.
@(export)
...
The main program will load the game DLL and setup the game by calling the `game_init` procedure of the game DLL. It will then go into a main loop and each frame call the game DLL's `game_update` procedure, if that procedure returns `false` then the main loop will terminate. After the update the main loop will check if the time `game.dll` was written (modified) is the same time as when the current game DLL was loaded. If it differs, then it tries to load the newly modified game DLL. If it succeeds it will then fetch the pointer from the old game DLL using the `game_memory` procedure and pass that to the new game DLL using the `game_hot_reloaded` procedure.
...
// The main program loads a game DLL and checks once per frame if it changed.
...
main :: proc() {
    // game_api_version is used to number the loaded game DLL. It is incremented
    ...
    for {
        // The game API's update procedure will update and render the game.
        ...
        if last_game_write_err == os.ERROR_NONE && game_api.lib_write_time != last_game_write {
            ...
            // will be false and we will try again next frame.
            if new_game_api, new_game_api_ok := load_game_api(game_api_version); new_game_api_ok {
                // Fetches a pointer to the game memory in the OLD game DLL
                game_memory := game_api.memory()

                // Completely unload the old game DLL. The game memory survives,
                // it will only be deallocated if we explicitly free it.
                unload_game_api(game_api)

                // Replace the game_api with the new one, now any call such
                // as game_api.update() will use the new code.
                game_api = new_game_api

                // Tell the new game API to use the old one's game memory.
    ...
    // This will deallocate game memory and do other cleanup.
    game_api.shutdown()
    unload_game_api(game_api)
}
...
// Loads the game DLL and returns a GameAPI object that contains pointers
// to all the required procedures of the game DLL.
load_game_api :: proc(api_version: int) -> (GameAPI, bool) {
    ...
    // to copy the DLL before the odin compiler has finished writing it.
    // In that case we will return and try again the next frame.
    //
    ...
    // Note that it manually cast them to the correct procedure signature.
    api := GameAPI {
...
While you could fully reset your game by shutting the main program off and on again, it is quite useful to have an in-game way to completely reset it without having to restart the program. This is useful if you have ended up in a bad state or just want to start over your play testing. I do something like follows.
...
Thanks for reading! Hot reload has been a great help to make my creative coding flow better, and I hope it will help you too!
