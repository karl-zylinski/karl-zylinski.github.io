...
[Raylib](https://www.raylib.com/) is a code library for drawing graphics, creating windows, processing user input keyboard, mouse and gamepads, and also outputting sound. It's a simple and easy-to-use library created for learning game programming, so it's a great fit for your first game.
...
> **NOTE:** If you chose to not use Sublime Text then this section won't make much sense. In that case you can compile the game by opening a command-line and going to the `c:\code\my_first_game` directory and from there running `c:\odin\odin run .`

A Build System is small file that instructs Sublime how to compile our game. In Sublime, click the menu button and go to Tools -> Build System -> New Build System...
...
Let's start by looking at the line `main :: proc() {`. This line defines a new function called `main`. In Odin, functions are called procedures (shortened proc), which we will call them from now on. When an Odin program starts, it looks for a proc called `main` and starts the program from the top of that proc. The proc runs from the opening curly brace `{`, to the matching closing curly brace `}`, as shown below:
