...
Now, don't go put `#soa` everywhere just because you can. It will still make the code a bit trickier to write (you need to use `#soa` pointers etc instead of normal pointers). The code will also be a bit harder to debug. Put it in if you have proof of a clear performance benefit.
